...
def _clean_python_string(value: str) -> str:
...
def _write_text_file(text: str, filename: str) -> str:
    out_dir = Path(tempfile.mkdtemp(prefix="lprobe_app_"))
    out_path = out_dir / filename
...
def generate_opentrons_protocol(
    robot_input_filename: str,
    protocol_name: str,
    transfer_volume: float = 10,
    source_labware: str = "idtedeep_96_wellplate_1000ul",
...
def load_labware(protocol):
    ...
    tipracks = [protocol.load_labware('opentrons_96_filtertiprack_20ul', location=str(i)) for i in range(7, 12)]
    return plate_dict, tipracks


def load_instruments(protocol, tipracks):
...
def run(protocol):
    plate_dict, tipracks = load_labware(protocol)
    ...
    for cycle_index, cycle in enumerate(cycle_data):
        for i in range(0, len(cycle), 96):
            sublist = cycle[i:i + 96]
            plate_source = plate_dict[str((i // 96) + 2)]
            transfer_liquid(pipette, sublist, plate_source, block_destinations[cycle_index])
"""
...
def run_protocol_workflow(
    ...
    protocol_text = generate_opentrons_protocol(
        robot_input_filename=str(robot_input_filename).strip(),
        protocol_name=str(protocol_name).strip() or f"L-probe cherry picking {clean_tag}",
        transfer_volume=float(transfer_volume),
        source_labware=str(source_labware).strip(),
...
def run_full_workflow(
    ...
    robot_input_filename = f"robot_input_plate_{starting_id}_{clean_tag}.csv"
    protocol_filename = f"opentrons_cherry_picking_plate_{starting_id}_{clean_tag}.py"
    protocol_text = generate_opentrons_protocol(
    ...
    files = [
        ...
        (out_dir / robot_input_filename, robot_input, False, False),
    ...
    protocol_path = out_dir / protocol_filename
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zip_file:
        ...
        zip_file.write(protocol_path, arcname=protocol_path.name)
...
with gr.Blocks(title="L-probe Barcode Tools") as demo:
    ...
    with gr.Group(visible=False) as protocol_panel:
        ...
        protocol_tag = gr.Textbox(label="Output tag", value="standard_barcoding_scheme")
        protocol_volume = gr.Number(label="Transfer volume (µL)", value=10, precision=1)
        protocol_source_labware = gr.Textbox(label="Source plate labware", value="idtedeep_96_wellplate_1000ul")
        protocol_destination_labware = gr.Textbox(
            label="Destination labware",
            value="opentrons_24_tuberack_eppendorf_2ml_safelock_snapcap",
        )
        protocol_pipette = gr.Textbox(label="Pipette", value="p20_single_gen2")
        protocol_mount = gr.Radio(["left", "right"], label="Pipette mount", value="left")
        protocol_button = gr.Button("Generate protocol", variant="primary")
        protocol_summary = gr.Textbox(label="Summary", interactive=False)
        protocol_preview = gr.Code(label="Protocol preview", language="python")
        protocol_download = gr.File(label="Download Opentrons protocol")
    ...
    def _show_workflow(selected):
        return (
            ...
            gr.update(visible=selected == "Generate Opentrons protocol"),
    ...
    workflow.change(
        _show_workflow,
        inputs=workflow,
        outputs=[full_panel, assign_panel, robot_panel, protocol_panel, explain_panel, verify_panel],
    )
    ...
    protocol_button.click(
        ...
        inputs=[
            ...
            protocol_tag,
            protocol_volume,
            protocol_source_labware,
            protocol_destination_labware,
            protocol_pipette,
            protocol_mount,
        ],
        outputs=[protocol_summary, protocol_preview, protocol_download],
    )
    explain_file.change(
        get_first_lbar_id,
